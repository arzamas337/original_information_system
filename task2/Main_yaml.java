...
public class Main_yaml {
    public static void main(String[] args) {
        System.setOut(new PrintStream(System.out, true, StandardCharsets.UTF_8));
        Scanner scanner = new Scanner(System.in);
        MyClient_rep_yaml myClient_rep_yaml = new MyClient_rep_yaml(); // Работа с YAML
        ...
        while (true) {
            ...
            try {
                switch (choice) {
                    case 1 -> {
                        List<Client> clients = myClient_rep_yaml.readAll();
                    ...
                    case 2 -> {
                        ...
                        myClient_rep_yaml.add(new Client(org, type, addr, phone, contact));
                        System.out.println("Клиент добавлен.");
                    }
                    case 3 -> {
                        System.out.print("Введите ID клиента: ");
                        int id = scanner.nextInt();
                        Client client = myClient_rep_yaml.getById(id);
                        System.out.println(client != null ? client.toStringFull() : "Клиент не найден.");
                    }
                    case 4 -> {
                        System.out.print("Введите ID клиента для удаления: ");
                        int delId = scanner.nextInt();
                        System.out.println(myClient_rep_yaml.delete(delId) ? "Удалено." : "Клиент не найден.");
                    }
                    case 5 -> {
                        ...
                        boolean replaced = myClient_rep_yaml.replace(repId, new Client(org, type, addr, phone, contact));
                        System.out.println(replaced ? "Заменено." : "Клиент не найден.");
                    }
                    case 6 -> {
                        myClient_rep_yaml.sortByField("organizationName", true);
                    ...
                    case 7 -> {
                        ...
                        List<String> page = myClient_rep_yaml.get_k_n_short_list(k, n);
                        if (page.isEmpty()) System.out.println("Нет данных для отображения.");
                        else page.forEach(System.out::println);
                    }
                    case 8 -> System.out.println("Количество клиентов: " + myClient_rep_yaml.get_count());
                    case 0 -> {
